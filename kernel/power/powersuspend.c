...
struct workqueue_struct *suspend_work_queue_power;
...
static void set_power_suspend_state(int new_state)
{
	...
	if (!old_sleep && new_state == 1) {
		state = new_state;
		queue_work(suspend_work_queue_power, &power_suspend_work);
	} else if (!old_sleep || new_state == 0) {
		state = new_state;
		queue_work(suspend_work_queue_power, &power_resume_work);
	}
	spin_unlock_irqrestore(&state_lock, irqflags);
}
...
static int __init power_suspend_init(void)
{
	...
	suspend_work_queue_power = create_singlethread_workqueue("p-suspend");

	if (suspend_work_queue_power == NULL) {
		return -ENOMEM;
	}
	return 0;
}
...
static void __exit power_suspend_exit(void)
{
	...
	destroy_workqueue(suspend_work_queue_power);
} 
